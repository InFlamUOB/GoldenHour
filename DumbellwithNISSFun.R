...
DumbellwithNISSFun <- function(test_name,s,i){
...
source("RFANN.R",local=TRUE) #Random Forest and ANN functions load
...
saveRDS(see2, paste("Results/",test_name, "_AllLASSO.rds")) 
saveRDS(see2EN, paste("Results/",test_name, "_AllEN.rds")) 
...
plot1 <- ggplot(Total, aes(y=Names, x=Permuted_Data,xend=Real))+geom_dumbbell(color="cornflowerblue", size=1.5, point.colour.l="goldenrod1")+theme(axis.text.x = element_text(angle = 90), axis.text=element_text(size=12),axis.title=element_text(size=18),strip.text.x = element_text(size = 8, colour = "black"),axis.title.y = element_text(margin = margin(t = 0, r = 10, b = 0, l = 0)),axis.title.x = element_text(margin = margin(t = 10, r = 0, b = 0, l = 0)))+labs(x="AUC values",y="Selected Features")+xlim(0.45,0.9)
