...
source("/Users/laura/Documents/PhD/R/Pipeline/SettingsPipe.R")
source("/Users/laura/Documents/PhD/R/Pipeline/Names_better.R")

settings$number_of_cores <- detectCores()
test_name <- "FirstNISS" #Make sure this is changed between time points: Second, Third
settings$seed <- 132
...
#clean data
s <- s[,3:dim(s)[2]]
#Take away NISS and repeat, then store final means and things and plot them all together with facet?? Will 
#Will need more than a plot. Will need the values, so make sure they are stored differently with the Final Means 
#so all of them have the title and can trace it back -
...
settingsPipe$filter <- 75 #with NISS set it higher
...
for (i in 1:settingsPipe$loops){
  ...
  Betas2EN <- RegMeGeneral(xtrain, ytrain, alpha, nlambda)
  ...
  Betas2 <- RegMeGeneral(xtrain, ytrain, alpha, nlambda)
