...
Final_LASSO1<-filter(Final_LASSO,n>50) #threshold selected - 80%
...
Final <- unique(c(as.character(Final_EN1$All),as.character(Final_LASSO1$All)))

Final <- c("GPI","GIMAP4","LOC643373","AMD1","FLI1","VAT1","GPR84","SRXN1","HLA-A29.1","C3AR1","MYLIP","MANBA")
...
genes_2 <- c(Final)
# Tidy
mat <- dat[genes_2, match(clin$Array, colnames(dat))]
clin_ctrl <- clin[Group == 'Control']
clin_mods <- clin[Group == 'Critical'
                  ][, MODS := ifelse(MODS == 'Yes', 'MODS', 'Non MODS')
                    ][, MODS := relevel(as.factor(MODS), ref = 'Non MODS')]
...
# Plot
plot1<- ggplot(df, aes(Time, Expression, fill = Group)) + 
  geom_boxplot() +
  scale_fill_manual(values = c('#00BA38', '#619CFF', '#F8766D')) + 
  labs(title = 'Gene Expression by Label and Time Point', 
       x = 'Time', 
       y = 'Normalized Expression') +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text.x = element_text(angle = 45, hjust = 1)) +
  facet_wrap(~ Gene, nrow = 3, ncol = 4)

pdf(paste("Figures/SupplementaryFigure3.pdf"),width=8, height=4)
grid.arrange(plot1)
graphics.off()
